...
public class HomeActivity extends Activity implements View.OnClickListener {
    ...
    private void init(){
        ...
        tv_title=findViewById(R.id.tv_main_title); //获取界面标题控件
        tv_title.setText("手机安全卫士");        //设置界面标题
        ll_clean=findViewById(R.id.ll_clean); //获取手机清理布局
        ll_interception=findViewById(R.id.ll_interception); //获取骚扰拦截布局
        ll_security=findViewById(R.id.ll_security);          //获取病毒查杀布局
        ll_software_manager=findViewById(R.id.ll_software_manager);//获取软件管理布局
        rl_app_lock= findViewById(R.id.rl_app_lock);     //获取程序锁布局
        rl_speed_test= findViewById(R.id.rl_speed_test);//获取网速测试布局
        rl_netraffic=findViewById(R.id.rl_netraffic);   //获取流量监控布局
    ...
    private void getMemoryFromPhone() {
        ...
        total_sd=blockSize*totalBlocks;                     //获取内置SD卡总存储空间
        long availableBlocks = stat.getAvailableBlocksLong();//获取剩余存储块的数量
        avail_sd=blockSize*availableBlocks;                //获取内置SD卡剩余存储空间
        ...
        total_rom = memoryInfo.totalMem; //获取手机系统总内存
        avail_rom = memoryInfo.availMem; //获取手机系统剩余内存
        //计算内置SD卡使用量与总量的占比
    ...
    private class MyAsyncRomTask extends AsyncTask<Integer, Integer, Integer> {
        ...
        protected void onProgressUpdate(Integer... values) {
            ...
            pb_rom.setProgress((int)(values[0]));//设置ArcProgressBar的进度
        }
    }
